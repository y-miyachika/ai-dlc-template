---
name: sync-docs
description: 設計ドキュメント同期チェックエージェント。コード変更とドキュメントの乖離を検出し、更新を提案します。
tools:
  - read
  - edit
  - search
...
コード変更後に、設計ドキュメントとの乖離をチェックし、トレーサビリティを維持します。
...
```
@sync-docs [パッケージ名またはUnit番号]
```

例：
```
@sync-docs          # 全体チェック
@sync-docs api      # apiパッケージ
@sync-docs 001      # Unit 001
```
